...
public class ANW {
    ...
    ANW() {
        for (int i = 0; i < 12; i++) {
            String name;
            if (i < 10) {
                name = "0_Penguin_Running_00" + i + ".png";
            } else {
                name = "0_Penguin_Running_0" + i + ".png";
            }
            runpic[i] = new ImageIcon(this.getClass().getResource(name));
        }
        for (int i = 0; i < 6; i++) {
            String name = "0_Penguin_Sliding_00" + i + ".png";
            slidepic[i] = new ImageIcon(this.getClass().getResource(name));
        }
        for (int i = 0; i < 6; i++) {
            String name = "0_Penguin_Jump Start_00" + i + ".png";
            jumppic[i] = new ImageIcon(this.getClass().getResource(name));
        }
        for (int i = 0; i < 6; i++) {
            String name = "0_Penguin_Falling Down_00" + i + ".png";
